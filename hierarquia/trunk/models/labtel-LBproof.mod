...
# 2 - Somas do tráfego partindo de cada nó
param hs{s in I}  :=  sum{d in I: d!=s} D[s,d];

# 3 - Para cada nó, soma todo o tráfego partindo dos demais nós
...
var rb{i in I, j in I}, >= 0, <= 1 + mult*(gl-1);

# 2 - Congestionamento Máximo
var Hmax, >= 0;

# 3 - Parcela de tráfego vindo de "s" passando pelo enlece (i,j)
var h{ i in I, j in I,s in I: j!=s} >= 0, <= hs[s]*(1 + (mult)*(gl-1));
...
gl = sum{i in I: i!=j} rb[i,j];

# 4 - Grau Lógico de Saida
s.t. graulogout{i in I}: 
gl = sum{j in I: i!=j} rb[i,j];
