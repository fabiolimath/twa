...
void rede::StatisticaFTnet(const char* DATA, int gl, int k)
{
...
//   cerr << "\nnp = " << np;
  ...
  const char* dump = HmaxLP_API_FTnet_MODEL.c_str();
...
/*  cout << "\nTopologia: \n";
  ...
  if(lpx_simplex(LP)!=200) { printf("Simplex: ERRO!\n"); return;}	// Executa o simplex.
    ...
    double gap = apx((lpx_get_obj_val(LP)/FTnetLB[k][gl-1]) - 1);
    X[n] = gap;
    RLDA = gap;
    MaxA = gap;
    //   cout << "\n X[n] = " << X[n] << "";
...
//     cerr << "\nk = " << (k+1)%(Sn+1) << " - T[k] = " << T[k] << " - GL = " << gl;
    ...
    do
    {
      ...
      double gap = apx((lpx_get_obj_val(LP)/FTnetLB[k][gl-1]) - 1);
      X[n] = gap;
      if (gap < RLDA) RLDA = gap;
      if (gap > MaxA) MaxA = gap;
      somaX += X[n];
...
TimeSFTnet[k][gl-1] = tempo/CLOCKS_PER_SEC;
FTnetRLDA[k][gl - 1] = RLDA;
FTnetM[k][gl - 1] = X[0];
FTnetMn[k][gl - 1] = nm;
FTnetD[k][gl - 1] = S;
FTnetDn[k][gl - 1] = ns;
FTnetMaxA[k][gl - 1] = MaxA;
for(unsigned i = 1; i <= n; i++ ) FTnet[k][gl - 1][i-1] = X[i];

ofstream file;

string INFO_FILE(DATA);
INFO_FILE += ".FTnet.amostra";
...
/*  file << "\n" << DATA << "\n";
  file << "\nLB = " << FTnetLB[k][gl - 1];
  file << "\nHLDA = " << FTnetHLDA[k][gl - 1];
  file << "\nRLDA = " << FTnetRLDA[k][gl - 1];
  file << "\nMédia  = " << FTnetM[k][gl - 1];
  file << "\nDesvio Padrão = " << FTnetD[k][gl - 1];
  file << "\nMáximo Amostral = " << FTnetMaxA[k][gl - 1];
  file << "\nTamanho de Amostra para a Média  = " << FTnetMn[k][gl - 1];
  file << "\nTamanho de Amostra para o Desvio Padrão = " << FTnetDn[k][gl - 1];
  file << "\nAmostra:\n";*/
  for(unsigned i = 0; i < n; i++ ) file << FTnet[k][gl - 1][i] << "\n";
//   file << "\n\n";
  file.close();
}
...
ostringstream oss;
oss << "data/N" << NTOWNS << ".I" << Inst << ".s" << seed << "/" << NTOWNS << ".I" << Inst << ".s" << seed << ".FTnet.statistics";
string dir = oss.str();
...
if (file.is_open())
{
  ...
  file << T[k] << "    ";
  file << gl << "  ";
  file << TimeSFTnet[k][gl-1] << "\t";
  file << FTnetMn[k][gl - 1] << "\t";
  file << FTnetDn[k][gl - 1] << "\t";
  file << FTnetLB[k][gl - 1] << "\t";//
  file << setprecision(2) << FTnetRLDA[k][gl - 1] << "\t";
  file << FTnetMaxA[k][gl - 1] << "\t";
  file << FTnetM[k][gl - 1] << "\t";
  file << FTnetD[k][gl - 1];
//    << setprecision(4)
  file.close();
