...
/*     double gap = (lpx_get_obj_val(LP),PLB_PRECISION)/round(HmaxLB[k][gl-1],PLB_PRECISION)) - 1;
    ...
    double gap = apx((lpx_get_obj_val(LP)/HmaxLB[k][gl-1]) - 1);
    X[n] = gap;
    RLDA = gap;
    MaxA = gap;
...
/*      double gap = (round(lpx_get_obj_val(LP),PLB_PRECISION)/round(HmaxLB[k][gl-1],PLB_PRECISION)) - 1;
...
if (file.is_open())
{
  ...
  file << HmaxDn[k][gl - 1] << "\t";
  file << HmaxLB[k][gl - 1] << "\t";//
  file << HmaxHLDA[k][gl - 1] << "\t";
  file << setprecision(2) << HmaxRLDA[k][gl - 1] << "\t";
  file << HmaxMaxA[k][gl - 1] << "\t";
  file << HmaxM[k][gl - 1] << "\t";
  file << HmaxD[k][gl - 1];
//   << setprecision(4) 
  file.close();
