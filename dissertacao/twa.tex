...
A princípio, o número de variáveis binárias associadas aos componentes topológicos seria $N^3\cdotp W\cdotp K$ \cite{cormen02}, mas devemos excluir algumas
que são 
trivialmente nulas, aquelas com $i=n$, pois $i$ é origem da ligação lógica e $n$ é destino da ligação física. Isso resulta em $N^3\cdotp W\cdotp K -
N^2\cdotp K$.
...
O número de equações no modelo básico é $ 2\cdot N^2\cdot W + N^2 + N $, que em notação assintótica é $\Theta(N^2\cdot W)$ \cite{cormen02}. E somando o
...
como premissa, conclui-se a equação \ref{eq:ConserB}. 
...
ligações lógicas associadas ao par $(i,j)$ é o produto da capacidade de cada uma ($Cap$) pelo número de ligações lógicas entre $(i,j)$ no plano lógico $w$.
Este segundo fator é $Diff_{i,w}^j$, calculado na equação \ref{eq:ConserBdif}. Todo o tráfego passando pelas ligações lógicas $(i,j)$ nesse plano
deve ser limitado por $Cap\cdot
Diff_{i,w}^j$, o que de fato é feito pela Restrição \ref{rest:DefCapFlow}. 

Como o TWA não modela explicitamente as ligações lógicas entre o par $(i,j)$, também não fica definida a divisão de tráfego alocado nelas. Entretanto, do
que foi visto acima, conclui-se que sempre
haverá banda suficiente para atender ao tráfego alocado ao conjunto dessas ligações lógicas, respeitando à capacidade individual. A exata divisão do tráfego
...
Na forma básica do modelo TWA, podem aparecer ciclos nas rotas físicas. Isso poderia ser controlado adicionando a soma de todos os componentes topológicos
na função objetivo. Mas esses ciclos não interferem na modelagem e podem ser facilmente localizados e retirados em uma fase posterior do projeto.
...
ele é controlado indiretamente minimizando a função objetivo, por meio do custo de instalação ou outra métrica
...
Como foi comentado na seção anterior, a exata separação do tráfego nas ligações múltiplas em um dado
