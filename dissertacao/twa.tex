...
trivialmente nulas, aquelas com $i=n$, pois $i$ é origem da ligação lógica e $n$ é destino da ligação física.
...
facilitar a compreensão das restrições do modelo, que ficam independentes do dados de entrada. 
...
\begin{equation}
\sum_{s} q_{sw}^{iv}\cdot A_s \leqslant Cap\cdot \left(\sum_{m} B_{iw}^{mv} - \sum_{n} B_{iw}^{vn}\right) \Forall{(v,i,w)} \mbox{, com $v\neq i$}
...
\begin{equation}
\sum_{iw} q_{sw}^{iv} - \sum_{jw} q_{sw}^{vj} = Q_{sv} \Forall{(v,s)} \mbox{, com $v\neq s$}
\label{rest:ConservFlow}
\end{equation}
 
\end{itemize}

Nas próximas seções deste capítulo, as restrições da versão básica do modelo TWA serão analisadas detalhadamente. Outros casos de uso e extensões ao modelo
básico serão apresentados no capítulo seguinte. 
...
lógica são vistas na figura como planos paralelos que, quando sobrepostos, formam a topologia lógica. Esses planos serão aqui chamados de planos lógicos,
cada um associado a um comprimento de onda, onde um par $(i,j)$ pode ainda representar múltiplas ligações lógicas utilizando um mesmo $w$.  
...
componentes topológicos nos nós intermediários, semelhante a uma restrição de conservação de fluxo \cite{ramamurthy99}. A soma dos componentes das ligações
lógicas iniciadas em um nó $i$ no plano $w$, partindo de um nó intermediário $v$, deve ser menor ou igual à quantidade recebida. Isso é garantido se a
equação \ref{eq:ConserB}
for satisfeita.
...
componentes chegando e saindo de $v$, originados em $i$ no plano $w$. Por sua vez, a equação \ref{eq:ConserBdif} é equivalente à equação
...
deixa $v$. A soma dos componentes que chegam é $3$ e a dos que saem é $2$, portanto a conservação está mantida. Neste exemplo, como há
...
Ainda na Figura \ref{fig:Bconserv}, são mostradas duas possibilidades de interpretação dos componentes topológicos. Necessariamente duas ligações lógicas no
plano $w$ passam transparentemente por $v$, enquanto uma nele termina. Mas as informações presentes não permitem determinar por qual componente chega a
ligação lógica incidente em $v$. Isso poderia ser decidido analisando todo o conjunto de componentes das ligações lógicas originadas em $i$ no comprimento
de onda $w$. Mas pode não haver um único arranjo possível, ou seja, o TWA não modela o exato percurso físico das ligações lógicas em cada plano. Todavia,
isso não interfere na modelagem do restante do problema e não precisa ser resolvido nesta fase do projeto. De posse da solução provida pelo TWA, as
...
com poucas restrições e variáveis, ao mesmo tempo que representa uma limitação.
...
também pode ser definida em outra fase do projeto e não precisa ser modelada aqui, o que também contribui para a simplicidade do modelo. 
...
A Restrição \ref{rest:DefCapFlow} ainda acumula uma função que, por ser intuitiva, pode passar despercebida. Mas é fundamental para a consistência do
...
A Restrição \ref{rest:DefFis} acumula nas variáveis $D_{mn}$ a multiplicidade determinada pelos componentes topológicos. Dado um par $(m,n)$, as equações
...
valores maiores que necessário. Mas isso é controlado indiretamente minimizando a função objetivo, por meio do custo de instalação ou outra métrica
relacionada com a variável $D_{mn}$. Entretanto, no caso de $D_{mn}$ não influenciar na função objetivo, esse excesso não importaria para a solução e
poderia ser tratado em uma fase posterior do projeto. 
...
demandas de tráfego. Essas restrições são semelhantes às encontradas na modelagem agregada para o VTD \cite{ram02}. Além da separação do tráfego por
...
sutilmente diferente pois, em cada plano lógico $w$, um par $(i,j)$ representa um conjunto de ligações lógicas. 

Cada par $(i,j)$ é visto nas restrições de controle de fluxo como um único caminho, unindo todos os planos lógicos. Se ele representa na verdade múltiplas
ligações lógicas, a diferença é que terá uma capacidade maior de receber tráfego, que é controlada pela Restrição \ref{rest:DefCapFlow}. Por isso essas
restrições funcionam da mesma forma que \cite{ram96}. Portanto, são as restrições de conservação de fluxo que fazem a correlação ente os planos
lógicos.
...
de tráfego que iniciam na origem ($i=s=v$) seja igual a $1$ em todos os planos lógicos, ou seja, $100\%$ do tráfego originado em $v$. 

Por sua vez, a
Restrição \ref{rest:ConservFlow} garante que o tráfego emitido seja encaminhado através da rede e entregue no destino. Para cada nó intermediário $v$ da
rede, fixado uma origem de tráfego $s$ ($v\neq s$), a porção de tráfego que deve ser entregue é $Q_{sv}$. Ela é igual à soma do tráfego chegando por todos
os planos lógicos $w$, vindo de qualquer nó intermediário $i$, subtraída da soma do tráfego partindo com destino destino a qualquer nó $j$, em qualquer
...
Como foi comentado na seção anterior, a exata separação do tráfego nas ligações múltiplas em dado
plano não é modelada, mas isso não influencia na conservação do fluxo pela rede. Considere um plano $w$, com um par $(i,j)$ que represente múltiplas
ligações lógicas. Se uma determinada fração de fluxo de tráfego for alocada a esse par, em uma fase posterior do projeto, ela pode ser subdividida e 
transportada simultaneamente por mais de uma ligação lógica entre o par $(i,j)$, no plano $w$. Neste caso, como as rotas físicas terão o mesmo comprimento
de onda, eles não compartilham ligações físicas ao longo do percurso. Mas essas rotas podem ainda não ser disjuntas, pois é possível compartilharem nós
intermediários. Essa subdivisão pode levar em conta métricas não tratadas aqui, como o fator BL \cite{Agrawal97}. 
