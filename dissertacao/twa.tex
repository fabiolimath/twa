...
\chapter{Traffic over Wavelength Assignment}
\label{Basic}
...
A definição dos componentes topológicos não deixa claro aonde terminam as ligações lógicas. Sua finalização será garantida 
...
Se o número de componentes topológicos incidentes em $m$ for maior que o número de iniciados, não originados nele, essa diferença é 
o número de ligações lógicas que terminam em $m$. É deste modo que a finalização das ligações lógicas pode ser mapeada. Isso 
assegura a rastreabilidade das ligações lógicas desde sua origem, a partir dos componentes topológicos agregados.
...
quantidade de componentes topológicos incidentes em $j$ ({\footnotesize$\sum_{mw}B_{iw}^{mj}$}), diminuído do número de componentes
topológicos iniciadas em $j$ ({\footnotesize$\sum_{nw}B_{iw}^{jn}$}).
...
é necessário novas incógnitas. Para este fim, é definida a Variável \ref{FisVar}, que registra em $D_{mn}$ a multiplicidade física alcançada pelos
componentes topológicos. Se $D_{mn}=0$, não há ligações físicas entre o 
par $(m,n)$, mas se $D_{mn}=k$, para algum $k \in \{0,..,K\}$, existem $k$ ligações físicas entre o par $(m,n)$.

Pela forma como $D_{mn}$ é calculada na Restrição (\ref{FisVar}), a rigor, seu valor poderia ser maior do que é determinado pelos componentes topológicos.
Mas
isso não ocorre quando o número de ligações físicas for minimizado na função objetivo.

Se $D_{mn}$ for dado de entrada do problema, a Restrição (\ref{DefFis}) limita a multiplicidade física dos componentes topológicos 
...
Como o número de componentes topológicos é $N³\cdot W\cdot K$ e o número de restrições é $\Theta(N²\cdotp W)$, o número de variáveis do modelo é da ordem de
...
componentes topológicos será $N\cdotp H\cdotp W$. Supondo uma topologia física conexa, temos $H>N$ \cite{cormen02}. Entretanto, é razoável
supor
