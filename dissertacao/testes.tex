...
operacionais e demais ferramentas computacionais utilizadas neste trabalho, como é decrito no Capítulo \ref{cap:ferramentas}.
...
As principais diferenças entre o AW e o TWA são que o modelo AW não faz nenhum tipo de agregação de variáveis e separa cada aspécto do projeto em variáveis
...
O AW
é mais abrangente que o TWA em alguns aspectos, mas em outros não. A princial vantagem dele, em relação ao TWA, é que as rotas físicas e a distribuição do
...
ser controlada, apezar disso não ter sido explorado em \cite{Karcius04}. Além disso, não foi prevista em \cite{Karcius04} a possibilidade da topologia
física ser uma das variáveis do problema, e nem dela possuir multiplicidade de ligações físicas. 

As restrições do AW que tratam do VTD não suportam multiplicidade de ligações lógicas, especicamente a restrição \ref{aw:rest:totFlow2}, pois geraria mais
tráfego do que existe na matriz de demandas $P_{sd}$. Para este fim, seria necessária uma restrição de limitação de capacida nos moldes da Restrição
\ref{rest:DefCapFlow} do TWA. No AW não há uma restrição uma restrição com funcionalidade equivalente. Em razão da toplogia lógica não possuir
...
limitação. Na relação a seguir são compardos paralelamente os modelos AW e TWA, em termos de funcionalidade.
...
\subsection{Resultados com modelo AW que serão alvo de comparação}
...
lógica, que serão escolhídas pelo HLDA e passadas aos modelos derivados como um dado de entrada. Definida a topologia lógica, a distribuição do tráfego é um
problema de programação linear de fácil resolução \cite{ram02}, deixado para ser resolvido em uma fase posterior do projeto. 

Sem a distribuição de tráfego, os modelos derivados são modelos de proramação interia, e não mais um MILP como o AW. Além disso, como a
...
O HLDA recebe como entrada a matriz de demandas de tráfego e um grau lógico para a rede. Com isso ele constroe uma toplogia lógica, sem fazer distribuição
de tráfego. Assim, o grau lógico $G$ define as instâncias nos testes. Definida a toplogia lógica pelo HLDA, o modelo AW-$l$ é utilizado para determinar as
rotas físicas, minimizando $L$. A solução para as rotas físicas é passada ao modelo AW-$s$, para reotimização, fixado $L$. Nesta fase, à instância é passado
também um limite $W$, o máximo de comprimentos de onda que poderão ser usados. E no final é registrada a quanidade de comprimentos de onda de fato
...
ao $L$, à topologia lógica e ao $W$ fixados, retirando os ciclos na solução final. As metricas de interesse nessa abordagem são listadas a seguir, com uma
breve decrição de seu relacionamento como o étodo proposto em \cite{Karcius04}. 
...
\begin{enumerate}
	...
	\item $W$: não é minimizado diretamente na reotimização (etapa $3$), ele é limitado em cada instância, de modo a obter uma solução viável que atenda
às rotas físicas, para $L$ fixado;
	\item $S$: não é uma métrica de particular interesse, foi minimizada apenas para remover cliclos nas rotas físicas na etapa $3$.
\end{enumerate}
...
% 	\item Por fim, a solução do AW-$l$ é passada ao modelo AW-$s$ para reotimização e alocação de comprimentos de onda, com um dado limite $W$. Nesta
% fase $S$ é minimizado para etirar ciclos
% \end{enumerate}
...
utilizou-se o modelo AW, descrito na seção anterior. Nos resultados que iremos confrontar, são considerados: o número de ligações lógicas ($L$) e o
...
controle do grau lógico (Restrição \ref{ConservGl}) e a de limitação do número de ligações lógicas em cada ligação física (Restrição \ref{L}). Esta
formulação específica é denominada de TWA-$a$. 
...
\end{itemize}
     ...
     Para controlar a qualidade das soluções quanto ao congestionamento, foram obtidas topologias virtuais com
uma implementação da heurística HLDA. Para cada uma destas topologias, foi distribuído o tráfego e calculado o congestionamento através do 
 do GLPK, utilizando uma versão do modelo clássico para o VTD \cite{ram02}. O valor de congestionamento obtido, arredondado para cima, foi usado como a
capacidade dos canais  lógicos, na Restrição \ref{rest:DefCapFlow}. Para cada instância, esse procedimento levou menos de um segundo, portanto não será 
...
ligações lógicas ($W=1$ e $L=1$), minimizando o número de saltos ($S$). Um solver para problemas MILP é instanciado com essa configuração. 
...
fica $L$, pois, minimizando-o, maximizamos a disponibilidade da rede.
 ...
 Utilizamos o SCIP para encontrar as soluções viáveis. Além de calcular a capacidade dos canais ópticos ($Cap$), como foi descrito acima, o GLPK também foi
usado para interpretar o modelo AMPL, gerando a entrada de dados para o SCIP. Os resultados que serão confrontados com os nossos foram produzidos com o ILOG
...
Os resultados para a rede de $6$ nós foram compilados na Tabela \ref{N6}. Na Figura \ref{fig:Karcius-rede6} está representada a topologia física da rede de
$6$
nós, e na Figura \ref{fig:Karcius-rede6-demandas} sua matriz de demandas de tráfego \cite{Karcius04}. A primeira coluna registra o grau lógico de cada
instância
...
\begin{tabular}{c|ccc|ccc|ccc|} %\\ \hline
    &  \multicolumn{3}{c}{AW} \vline  & \multicolumn{6}{c}{TWA-$a$} \\ \hline
...
ópticos. O tráfego é roteado pela topologia virtual utilizando uma formulação linear (LP) consistindo somente das restrições de tráfego do MILP relaxado. 
