...
A modelagem encontrada em \cite{Karcius04} é baseada nas modelagens clássicas dos problemas VTD e RWA \cite{Zang00, ram02}.
...
subproblemas do projeto de uma WRON. A topologia física é considerada conhecida, sendo passada como parâmetro para o modelo. Além disso, é suposto que ela
...
\begin{equation}
\sum_j  b_{ij}  \leqslant Gout_i \Forall{i}
\label{aw:rest:lg1}
\end{equation}

\begin{equation}
\sum_i  b_{ij}  \leqslant Gin_j \Forall{j}
\label{aw:rest:gl2}
\end{equation}
...
\caption{Número de variáveis binárias, reais e equações no modelo AW.}
...
\item AW-$l$: Máximo de Ligações Lógicas em Cada Ligação Física:
\begin{equation}
\mbox{Minimize:} \quad L
\label{aw:fo:L}
\end{equation}


\item AW-$s$: Número de Saltos Físicos:
\begin{equation}
\mbox{Minimize:} \quad \sum_{mnij} p_{mn}^{ij} = S
\label{aw:fo:B}
\end{equation}
...
multiplicidade de ligações \cite{ram02}. Assim, as Variáveis de \ref{aw:vars}.2 a \ref{aw:vars}.4 podem passar a suportar multiplicidade de ligações
lógicas. O que de fato é assumido em \cite{Karcius04}.
...
\label{cap:testes-sec:CompKarcius}
...
quanto às métricas de interesse. São elas: o número de ligações lógicas ($L$) e comprimentos de onda ($W$),  disponíveis em cada ligação física; o grau
lógico da rede ($G$); o número de saltos físicos na topologia ($S$); e o
...
de ligações lógicas é usada a Restrição \ref{L}, adequada para topologias físicas sem multiplicidade. Além disso, a definição dos componentes
...
alvo de comparação. Esta formulação específica, apresentada a seguir, é denominada de TWA-$sl$. 
    ...
    Nos resultados apresentados em \cite{Karcius04} não é calculado o congestionamento, apenas é adotada a topologia lógica produzida pela heurística HLDA.
Deste modo, para os resultados apresentados neste seção, foram obtidas topologias lógicas com uma implementação da heurística HLDA.
...
considerado na contagem de tempo de processamento dos resultados apresentados mais adiante nesta seção.
...
\item Número de Saltos Físicos

\begin{equation} 
   \mbox{Minimize:} \quad   \sum_{imnw} B_{iw}^{mn} = S
\label{testes:fo:B}
\end{equation}
...
muitos outros tipos de problemas de otimização, pode-se levar mais tempo determinando a otimalidade de uma solução viável já encontrada, do que aquele
gasto para obté-la. 
...
$G$  & Grau Lógico\\
$L$   & Máximo de Ligações Lógicas nas Ligações Físicas\\
$W$   & Número de Comprimentos de Onda Utilizados\\
$S$   & Número de Saltos Físicos\\
$t$   & Tempo em segundos para encontrar a primeira solução viável\\
$Cap$ & Capacidade de Tráfego de cada Ligação Lógica\\
$I$   & Número  de instâncias insolúveis visitadas\\ \hline
...
\begin{figure}[htb!]
	\centering
	\includegraphics[bb=0 0 197 107,scale=1.0]{./figs/AW_solucao.N6.eps}
	% AW_solucao.N6.eps: 197x107 pixel, 72dpi, 6.95x3.77 cm, bb=0 0 197 107
	\caption{Rotas físicas da solução para a rede de $6$ nós, com grau lógico $1$.}
	\label{fig:AW_solucao.N6}
\end{figure}

A Figura \ref{fig:AW_solucao.N6} apresenta as rotas físicas da solução obtida para a rede de $6$ nós, com grau lógico $1$. Ela apresenta o mínimo possível
de ligações físicas utilizadas em uma solução conexa, ou seja, um anel. Note que não há passagem transparente em nenhum nó, portanto essa topologia é opaca.
Como o número de saltos foi minimizado na função objetivo, é natural que a solução ótima tenda a não possuir ligações transparentes, pois estas usam mais de
um salto para realizar a ligação lógica. Apenas nas soluções para grau lógico $4$ e $5$ ocorreram passagens transparentes, mas não convém exibi-las pelo
elevado número de saltos, $25$ e $46$, respectivamente.
...
representada
a topologia física da rede de $12$ nós, e na Figura \ref{fig:Karcius-rede12-demandas} sua matriz de demandas de tráfego \cite{Karcius04}. Desta vez temos
$6$
