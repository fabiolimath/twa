...
conversão entre comprimentos de onda, o número de desvios equivale a quantidade de conversões. %no projeto. 
...
para o plano $w$ é limitado pela quantidade de ligações físicas saindo de $v$. Esse limite é chamado de grau físico de saída de $v$ ($GFout_v$). Ele definie
o escopo da Variável \ref{x} ($x_{iv}^w$). Ela guarda o número de desvios em $v$ destinados ao plano $w$, das rotas com origem $i$. Como $x_{iv}^w$ é uma
...
pelo seu limite natural ($GFout_v$), adotou-se a constante $CON_v$, que que pode ser menor que $GFout_v$. No Conjunto de Dados \ref{dados:desvios} é
definida $CON_v$ e também o limite $TCON_v$, que limita o total de conversões em cada nó, que é o foco do interesse nesta seção.
...
dispensáveis à modelagem. Por exemplo, elas poderiam ser substituídas pela adição da soma de todas as conversões na função objetivo do modelo básico. Pois a
