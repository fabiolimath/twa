...
\begin{equation}
\sum_{wn} B_{vw}^{vn} \leqslant Gout_v\Forall{v}
\label{cases:rest:GLout}
\end{equation} 

\begin{equation}
\sum_{iwm} B_{iw}^{mv} - \sum_{iwn} B_{iw}^{vn} \leqslant Gin_v\Forall{v},\, \mbox{\small$i\neq v$} 
...
\begin{equation}
  \sum_{iw}LL_{iv}^w \leqslant  Gin_v\Forall{v},\, \mbox{\small$i\neq v$} 
   \label{eq:Diffv}
\end{equation}
...
\begin{figure}[htb]
	\centering
	\includegraphics[bb=0 0 320 132,scale=0.7]{./figs/B_GL_Multi.eps}
	% B_GL_Multi.eps: 0x0 pixel, 300dpi, 0.00x0.00 cm, bb=110 613 432 747
	\caption{Exemplo com Grau Lógico de Entrada e Saída iguais.}
	\label{fig:B_GL}
\end{figure}

A Figura \ref{fig:B_GL} apresenta um exemplo onde o nó $v$ tem grau lógico de entrada e saída iguais a $2$. Dentre os componentes que partem de $v$, um
deles compõe uma ligação lógica iniciada em $i_2$, que não está representado na figura, bem como o destino desta ligação, que passa transparentemente por
$v$. Apenas um componente com valor $2$ inicia ligações lógicas em $v$, por isso $Gout_v = 2$. Por sua vez, $2$ componentes incidem em $v$ trasportando
...
\begin{notation}
O índice $r\in\{1,\cdots,CapLog_{ij}\}$ enumera os possíveis múltiplos canais lógicos entre o par $(i,j)$, onde $CapLog_{ij}$ 
...
$f_{ij}^r$. A Restrição \ref{rest:DefCapFlow} é repetida na equação \ref{eq:DefCapFlowBasic} para facilitar a compreensão desse relacionamento. As variáveis
...
sobre as frações de fluxo $q_{sw}^{ij}$, somando todo o tráfego que passa pela ligação lógica $(i,j)$, onde agora não há multiplicidade. Deste modo, o
...
Uma terceira forma para se controlar diretamente o congestionamento é adotando a Restrição
...
\item Máximo de Ligações Lógicas em Cada Ligação Física
...
\ref{cases:rest:LimXw}. As Restrições \ref{cases:rest:LimX} e \ref{cases:rest:LimXw} aplicam os limites $TCON_v$ e $CON_v$, sendo ambos opcionais;
