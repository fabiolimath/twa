...
Como foi comentado na Seção \ref{cap:twa-sec:limitacoes}, a Restrição \ref{rest:DefFis} permite que haja excessos na variável de topologia física
$D_{mn}$, que são indiretamente controlados pela função objetivo do modelo básico. Para facilitar a leitura desta seção, a
Restrição \ref{rest:DefFis} é repetida na equação \ref{eq:cases:DefFis}. Se a variável de topologia física $D_{m,n}$ não for minimizada direta ou
indiretamente na função objetivo, pode-se usar a equação \ref{eq:CtrlFis} para anular as ligações físicas não utilizadas. Entretanto, quando houverem
ligações físicas utilizadas entre o par $(m,n)$, ainda seria possível que $D_{mn}$ registre valores maiores que o necessário para atender aos componentes
topológicos, conforme foi comentado na Seção \ref{cap:twa-sec:limitacoes} a respeito da Restrição \ref{rest:DefFis}. Mas, se $D_{mn}$ não influencia na
função objetivo, esse excesso poderá ser retirado analisando a solução obtida.
...
Outra alternativa é controlar a multiplicidade das ligações lógicas por plano lógico ($PMl$), o que é feito pela Restrição \ref{cases:rest:PMl}.
...
ligações lógicas iniciadas em $i_1$ e $i_2$, cuja soma é $3$, mas como uma passa transparentemente vindo de $i_2$, então $GLin_v = 2$. Os componentes que
...
binárias adicionadas ao modelo seria $N^2\cdot GL$, idem para variáveis contínuas. Supondo ainda que $GL < N$, o que é bem razoável, não haveria alteração
...
de entrada, por exemplo, uma parte irá passar transparentemente. Então, a quantidade deligações lógicas incidentes por esta fibra também está limitada por
...
Se a topologia física for pré-definida como dado do problema, diz-se que a topologia física é fixa. Neste caso, há uma forma alternativa para se modelar
...
com relação ao comprimento de onda de destino na conversão. Na separação da topologia lógica por comprimento de onda, introduzida na Seção
...
conversão entre comprimentos de onda, não será conhecido explicitamente em qual plano essa rota iniciou seu trajeto. As restrições propostas é que
deverão garantir que haja tal conservação do trajeto entre os planos. Portanto, não será modelada a conversão diretamente, mas apenas o desvio da rota e a
conservação dos percursos. Como cada desvio corresponde univocamente a uma conversão entre comprimentos de onda, o número de desvios equivale a quantidade
de conversões no projeto. 

No Conjunto de Dados \ref{dados:desvios} são definidos os limites para o número de conversões em cada nó da rede ($TCON_v$) e o número de conversões para
um mesmo comprimento de onda ($CON_{v}$). Este segundo, mesmo se não for de interesse em um caso de uso particular, é necessário para limitar o escopo da
variável \ref{x}, que guarda o número de desvios para o plano $w$ em $v$, das rotas com origem $i$. Como $x_{iv}^w$ é uma variável inteira, para não
prejudicar a eficiência do modelo, convém adotar um valor pequeno para $CON_v$. Pois quanto maior o domínio de uma variável inteira, maior será o número
de variáveis binárias associadas a ela.
...
\begin{var}
 \label{x}
Em um nó intermediário $v$, o número de rotas iniciadas em $i$ que são desviadas para o plano $w$ é $x_{iv}^w \in \{0, \cdots, CON_{v}\}$. 
\end{var}

A Restrição \ref{rest:DefCapFlow} do modelo básico é substituída, pelas restrições \ref{cases:rest:Desvios} e \ref{cases:rest:ConservBgeral}. Dentre estas
duas, a primeira assume a função de conservação dos comprimentos de onda ao londo das rotas, além de controlar os desvios de plano, que correspondem às
...
A restrição \ref{cases:rest:CtrlX} limita o número de desvios pelo que a topologia física é capaz de prover. As Restrições \ref{cases:rest:LimX} e
\ref{cases:rest:LimXw} aplicam os limites $TCON_v$ e $CON_v$, sendo ambas opcionais, dispensáveis à modelagem. Por exemplo, elas poderiam ser substituídas
pela adição da soma de todas as conversões na função objetivo do modelo básico. Pois a quantidade de conversões também influencia no dimensionamento dos
nós.
...
Em cada ligação física só pode passar uma ligação lógica utilizando o comprimento de onda $w$. Assim, em um nó $v$, o número de rotas que são desviadas
para o plano $w$ é limitado pela quantidade de ligações físicas saindo de $v$. Essa é a função da restrição \ref{cases:rest:CtrlX}. A restrição
\ref{cases:rest:LimXw} tem o mesmo papel, mas dessa vez adequando o número de conversões à $CON_v$. Apesar dele ser o limite de cada variável $x_{iv}^w$,
esse limite é compartilhado por todas as rotas, independente da origem $i$. Se a topologia física for fixa e $CON_v \leqslant \sum_n D_{vn}$, a
Restrição \ref{cases:rest:CtrlX} é satisfeita pela Restrição \ref{cases:rest:LimXw}, que a substitui. 
...
Na Figura \ref{fig:desvios} há uma representação gráfica de duas
possibilidades para a conservação dos percursos, independente do comprimento de onda.
...
exige que outra restrição cuide da conservação dos comprimentos de onda. O que é feito pela Restrição \ref{cases:rest:Desvios}. Com o tráfego agregado por
comprimento de onda, o fator $LL_{iv}^w$ (Seção \ref{cap:twa-sec:Bconserv_Cap}) perde seu significado. Pois agora só importa o número de ligações lógicas
entre o par $(i,v)$ considerando todos os comprimentos de onda. 
