...
\zz
\begin{proposition}
Se a topologia física é uma variável do modelo ($D_{m,n}$), o grau físico de entrada e saída $H$ é considerado uniforme para 
...
No modelo TWA o número de ligações lógicas é implicitamente limitado por $CapLog_m$. Todavia, o número de receptores ou 
...
indiretamente limitada por $CapLog_m$ ou pelo grau lógico. Para não usar multiplicidade nas ligações lógicas, basta fazer 
$Ml=1$.


\zz
\begin{proposition}
 Grau Lógico $=$ $Gl\leq CapLog_m, \quad \forall m$.
\label{Gl}
\end{proposition}
...
é o mínimo entre $CapLog_m$ e $CapLog_n$. 
\end{notation}

\zz
\begin{theorem}
Fração de Tráfego $=$ $f_{i,r,j} \in [0,1]$: variável contínua.
 \label{f}
\end{theorem}

\zz
\begin{theorem}
Ligação Lógica $=$ $F_{i,r,j} \in \{0,1\}$: variável binária.
 \label{F}
...
A Restrição (\ref{DefChLog}) determina as ligações lógicas $F_{i,r,j}$ em termos das componentes topológicas. Em seguida, a 
...
física dos nós realizarem ligações lógicas ($CapLog_m$) será modificada para $H\cdot L$, se a topologia física é livre, ou 
$\sum_n D_{m,n}\cdot L$, caso a topologia física seja fixada.
