...
\label{testes}
Para avaliar a pertinência desta nova abordagem, testes computacionais foram realizados. Toda a modelagem do TWA, incluindo os casos de uso da 
Seção \ref{cases}, foi descrita em {\tt AMPL\rr} (\textit{A Modeling Language for Mathematical Programming} - www.ampl.com), de modo que facilmente 
...
controle do grau lógico (Seção \ref{ConservGl}) e a de limitação do número de ligações lógicas em cada fibra (Seção \ref{LimW}). Esta formulação 
...
Foram executados dois testes computacionais, com uma rede de $6$ nós e com uma rede de $12$ nós \cite{Karcius04}. Utilizamos o \textit{solver} 
