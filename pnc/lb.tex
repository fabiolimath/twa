...
heurísticas \cite{Sivarajan01,Nina07}. Toadavia, o cálculo de \textit{lower bounds} (LB) que garantam essa qualidade tem elevado custo computacional, sendo esta
a parte mais difícil dessa abordagem. Apresentamos aqui uma nova ténica para a obtenção de \textit{lower bounds} para o congestionamento que joga por terra
essa dificuldade. Ela é uma formula de cálculo direto, que denominamos \textit{Minimum Trafic Bound} (MTB), fornecendo um LB de alta qualidade para o
congestionamento, com custo computacional desprezível. 

% A segunda técnica, de propósito mais geral, é uma aprimoramento do método iterativo
% (ILB -
% \textit{Iterative Lower Bound}, encontrado em \cite{ram02}), transformando-o em uma
% técnica de busca mais eficiente, que chamaremos de \textit{Dichotomous Search Bound} (DSB).

% Para determinar um LB para o congestionamento, precisamos exergar qual é o mínimo de tráfego que pode ser encontrado em cada ligação lógica da rede. Não há uma
% resposta direta, mas podemos fazer uma estimativa olhando cada nó independentemente. 
% Na melhor das hipótesis, todo o tráfego que passa pelas ligações lógicas
...
% composto pelas demandas destinadas a ele. 

% Assim, dividindo todo o tráfego originado (incidente) em $m$ pelo máximo de ligações lógicas originadas (incidentes) em $m$, neste caso, $Gl$, temos o menor
% tráfego possível nessas ligações lógicas. Extrapolando isso para toda a rede, o maior dentre esses valores será um \textit{lower bound} para o congestionamento.
% Pois não é possível que um nó envie menos tráfego do que a soma das demandas originadas nele. Analogamente, não é possível que um nó receba menos tráfego do que
% o destinado a ele. O MTB é assim definido na equação (\ref{MTB}) como o mínimo dos valores calculados nas equações em (\ref{MT}). 
...
% informação que sobre a topologia da rede. Mas em modelagens mais abrangentes como o TWA, 
% 
% além de nos restringirmos ao VTD, consideraremos também a possibilidade de haver multiplas ligações lógicas entre um mesmo par de nós da
% rede. Sendo este o cenário em que poderíamos obter os menores valores possíveis para o congestionamento.  
...
\begin{theorem}
\label{MTB} 
Se o número de ligações lógicas iniciadas e finalizadas em cada nó da rede são parâmetros de entrada, então o MTB, definido no conjunto de Dados \ref{MT}, é um
\textit{lower bound} para o congestionamento no sub problema VTD.
\end{theorem}
...
seja, $\sum_n D_{in}$. Seja $\Psi_i$ a soma de todo o tráfego nas ligações lógicas iniciadas em $i$. Deste modo, $\Psi_i \geqslant \sum_n D_{in}$, conserando que
...
$(i,j)$. Assim, $\Phi_{ij} > \lambda_{max}^*$, o que é absurdo. Isso prova que a inequação (i) é verdadeira, e de modo análogo pode-se verificar
a validade da inequação (ii). \qed
...
Em última análise, o MTB explora a possibilidade da ligação lógica mais carregada da rede transportar predominatemente tráfego que não foi ou não será
retransmitido. Apezar de aparentemente ingênua, essa é uma suposição muito acertada, posto que na maioria dos testes feitos o MTB equivale
ao ótimo, como será visto na Seção \ref{testes}.


% 
% \begin{equation}
% OUT_m = \frac{\sum_n D_{mn}}{\alpha_m}, \quad \text{and} \quad IN_m = \frac{\sum_n D_{nm}}{\beta_m}, \quad \text{and} \quad Max_m = \max(OUT_m,IN_m) \Forall{m}.
% \label{MT}
% \end{equation}
% \begin{equation}
% MTB = \max Max_m
...
% qualquer $V$. Teriamos a restrição: 
% 
% \begin{equation}
% \label{hmaxv}
