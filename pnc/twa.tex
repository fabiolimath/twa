...
\sum_{sijw} T\cdot q_{sw}^{ij}\cdot A_s,\, \mbox{\small$i\neq s$} 
\label{MinC}
\end{equation} 
...
\begin{equation}
\text{\small Minimize: } \lambda_{max} \geqslant \sum_{sw} q_{sw}^{ij}\cdot A_s\Forall{(i,j)} \text{, } Ml=1
\label{Hmax} 
\end{equation}
% 
% \begin{equation}
