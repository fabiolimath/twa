...
\begin{figure}[htb!]
 ...
 \caption{Exemplo de topologia de rede em hierarquia.\textbf{\underline{ Mudar figura!!!}}}
...
tráfego esperadas. O projeto completo abrange aqui, além das variáveis de decisão suportadas pelo modelo TWA, a escolha da estrutura hieráquica da rede.

Idealmente, o projeto completo de uma malha hieráquica poderia ser feito incluindo restrições no TWA que modelassem a hierarquia, semelhante ao que foi
...
cruza de um nível hieráquico para outro, chamado de tráfego de acesso. Este tráfego pode ser acumulado nos nós de acesso ao \textit{backbone} (como
...
A escolha da estrutura hieráquica, como será definida, é um Problema de Cobertura de Conjuntos (PCC), com uma função objetivo especial. A formulação clássica
...
Tendo sido determinada a estrutura hieráquica, cada sub-rede é modelada com o TWA. Nesta fase, a estratégia adotada foi definir parâmetros de qualidade
...
Para o número de comprimentos de onda, foi utilizada uma estratégia adotada anteriormente em \cite{sbpo09}. Tentamos obter soluções com um comprimento
...
está apresentada na figura\ref{fig:descr.proc}. Existem três etapas, onde os resultados de cada uma são fornecidos para as seguintes.


\begin{figure}[!htb]
\begin{center}
\includegraphics[scale=0.7]{./figs/descr.proc.eps}
...
Na primeira etapa, são fornecidas as matrizes de demandas de tráfego e distâncias, únicos dados de entrada do projeto. Um algoritmo
genético faz a separação dos nós em sub-redes, tendo como função objetivo a minimização do tráfego transferido entre as sub-redes, ou seja,
procura-se concentrar o tráfego no interior das mesmas. Um resultado secundário dessa fase são as matrizes de distância e tráfego para cada sub-rede.

Na fase intermediária, são estimadas a média e o desvio padrão para congestionamento e tráfego retransmitido, para cada sub-rede, em graus lógicos variados.
Neste ponto também são calculados \textit{lower bounds} (LBs) e \textit{upper bounds} (UBs) para estas métricas. Os LBs são obtidos a partir de métodos
apresentados na literatura, os UBs são calculados a partir da média e do desvio padrão. Por fim, também são calculados UBs para o custo de instalação de cada
sub-rede.

A última etapa consiste na resolução do modelo TWA para as sub-redes, respeitando os \textit{bounds} calculados e visando utilizar o menor número possível de
comprimentos de onda. A saída desta última fase fornece a solução do projeto, representado pelas topologias física e virtual, roteamento e alocação dos
comprimentos de onda, além da distribuição do tráfego. Esses resultados são referentes às sub-redes em separado, porém, quando são agrupados, representam o
...
a rede possui $n$ nós, então o número máximo de \textit{clusters} e seu tamanho máximo ($M_a$) será igual a $\lfloor n/M_i \rfloor$. Assim, uma rede de $n$
nós e $k$ \textit{clusters}, é representada por dois vetores. Um vetor de tamanho $k$ contendo os tamanhos dos \textit{clusters} ($T=(t_1,\cdots,t_k)$) e outro
...
Para a escolha da estrutura hierárquica da rede foi implementado um algoritmo genético com a biblioteca {\itshape Galib}. A fim de representar todas as
possibilidades de configuração possíveis para a hierarquia, o cromossomo utilizado é um vetor de tamanho $n+M_a$ contendo uma permutação dos números inteiros
positivos até esse valor. A interpretação do cromossomo como uma hierarquia é feita da seguinte forma: 

\begin{enumerate}
\item Separa-se o cromossomo em dois vetores, mantendo-se a ordem em que ocorreram. O primeiro, de tamanho $n$, contém os números menores ou iguais à $n$. Este
é o vetor $P$, uma permutação dos nós da rede. O segundo vetor ($C=(c_1,\cdots,c_{M_a})$) contém os números maiores que $n$ e dará origem ao vetor de tamanhos
$T$. 
\item A partir de $c_1$, iterativamente, fazemos $t_i=[c_i \mod (M_a - M_i)] + M_i$, enquanto $n - s_i \geqslant M_i$. Ou seja, enquanto o número de nós
...
permutação de inteiros entre $1$ e $n+M_a$, sem uma estrutura particular, permitiu utilizarmos operadores genéticos pré disponíveis na biblioteca
\textit{Galib} e bem conhecidos na literatura \cite{ga2}. São eles os operadores genéticos do código $Ex26$ da \textit{Galib}, com a função de cruzamento:
\textit{Edge Recombination Crossover}. O algoritmo genético adotado foi o \textit{GADemeGA}, que mantém populações paralelas, com probabilidade de migração
...
satisfatório.
%, mas para implementações mais robustas é recomendado que sua calibragem seja melhor estudada.
...
Deste modo, a função objetivo do algoritmo genético foi definida como a soma da distância média entre os nós de cada grupo ($M$) com o total de tráfego
...
%com o total de tráfego retransmitido nos nós de acesso ao \textit{backbone} ($T$)

Primeiro calcula-se a distância média entre os nós de cada grupo, e a média desses valores é tomada como a distância média da hierarquia. Para calcular o
tráfego de acesso em um super-nó, soma-se todo o tráfego originado dentro do seu \textit{cluster} cujo destino são nós de outros \textit{clusters}, mais o
tráfego da condição inversa, ou seja, que origina-se em outros \textit{clusters} e possui destino no interior do \textit{cluster} a que pertence o referido
super-nó. Acumulando-se essa soma para todos os super-nós, temos o total de tráfego de acesso da rede.
...
caso $M$, por ser uma média, tem ordem de grandeza bem inferior à de $T$, que é uma soma simples. Portanto, para garantir o equilíbrio entre as duas métricas,
para cada instância um teste piloto deve ser feito para se estabelecer uma proporção que deixe $M$ e $T$ com a mesma ordem de grandeza. Feito isso, é possível
atribuir pesos para cada um, conforme for o interesse do estudo de caso em questão.
...
matrizes de distâncias são apenas um subconjunto da original. Enquanto que a matriz de demanda acumulam nos nós de acesso todo tráfego que deverá entrar no
...
interativo apontado por \cite[Seção 4.5]{estatistica}. Para a média, o intervalo de confiança pode ser obtido por \cite[Seção 4.4.1]{estatistica}:

$$e = Z_{(\alpha/2)}\cdot \frac{\sigma}{\sqrt{n}}$$

Onde $e$ é a margem de erro, a amplitude de intervalo de confiança, $Z_p$ é a distribuição normal reduzida, $\alpha$ é o nível de confiança exigido,
$\sigma$ é o desvio padrão populacional e
$n$ é o tamanho da amostra. Mas ao invés de estimar o intervalo de confiança, desejamos, a partir de uma margem de erro dada, estimar o tamanho de amostra
necessária. Resolvendo a equação acima para $n$, temos:
...
substituição pela sua estimativa amostral $s$ \cite[Seção 3.4.5]{estatistica}:
...
Procedemos de forma análoga para estimar o tamanho de amostra representativa para a estimativa do desvio padrão, cujo intervalo de pode ser obtido por
\cite[Seção 4.4.4]{estatistica}:
...
pela amostra. Neste
caso, vamos incrementando o tamanho da amostra até que $e'\leqslant e$, ou seja, até que o erro calculado seja menor ou igual ao exigido. 
...
$\sigma$ quando $n$ aumenta. Todavia, quanto maior for o nível de certeza exigido e quanto menor for erro aceito, maior será o tamanho de amostra
requerido.
...
Para estabelecer essa proporcionalidade, devemos obter uma razão adimensional que represente cada rede,
para então confrontá-las. Isso é feito na definição \ref{def:TIF}.

\begin{definicao}[TIF]
\label{def:TIF}
A Taxa de Instalação Física (TIF) é dada pela relação:
$$TIF = N\cdot \frac {\sum_{mn} C_{mn}\cdot D_{mn}}{\sum_{mn} C_{mn}}$$ 
\end{definicao}

Para justificar essa definição devemos considerar o quanto de ligações físicas foram estabelecidas ($FE = \sum_{mn} C_{mn}\cdot D_{mn}$), dentro do
total todas as possíveis ($FP = \sum_{mn} C_{mn}$). No entanto, a simples razão destes dois valores produziria uma relação que é proporcional ao tamanho
da
rede$^[$\footnote{Por abuso de notação, utilizamos $N$ para representar o tamanho da rede quando $n$ é usado como indexador.}$^]$ ($N$). Para corrigir
isso,
considerando que $FE$ é proporcional à $N$ e que o $FP$ é proporcional à $N^2$, uma relação adequada é $TIF = FE\cdot N
/FP$. 
...
essa TIF de referência como limitante superior. Ou ainda, pode-se embutir um \textit{gap}, exigindo que a rede projetada tenha TIF $20\%$ ou $30\%$ abaixo
da TIF de referência, por exemplo. Todavia, exigir um \textit{gap} muito grande pode acarretar em inviabilidade.
...
\begin{equation}
\sum_{mn} C_{mn}\cdot D_{mn} \leqslant TIF\cdot \sum_{mn} C_{mn}/N \mbox{, com } m<n.
\label{TIF}
\end{equation}
