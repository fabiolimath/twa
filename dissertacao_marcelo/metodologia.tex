...
Neste ponto também são calculados \textit{lower bounds} (LBs) e \textit{upper bounds} (UBs) para estas métricas. Os LBs calculados foram introduzidos na
dissertação de mestrado \cite{dissertFabio}, e os UBs são calculados a partir da média e do desvio padrão. Por fim, também são calculados UBs para o custo
de instalação físico de cada sub-rede.
...
Primeiro calcula-se a distância média entre os nós de cada grupo, e a média desses valores é tomada como a distância média da hierarquia. Neste ponto vale
resaltar que, dependendo do estudo de caso em questão, pode ser de interesse ponderar essas médias, possivelmente atribuindo maior pesso às distâncias do
\textit{backbone}.
