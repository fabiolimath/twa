...
A equação \ref{DefDistMed} define a fórmula de cálculo para $M$. Primeiro calcula-se a distância média entre os nós de cada grupo, \textit{backbone}
($DistMed_{bk}$) e \textit{clusters} ($DistMed_{cl}$), e a média desses valores é tomada como a distância média da hierarquia. Dependendo do estudo de caso em
questão, pode ser de interesse ponderar essas médias, possivelmente atribuindo maior peso às distâncias do \textit{backbone}. Na equação \ref{DefDistMed} esses
pesos ($PesoDist_{bk}$ e $PesoDist_{cl}$) estão apresentados multiplicando seus respectivos termos de distância média. O denominador no cálculo de $M$
...
\begin{equation}
M = \frac{DistMed_{bk}\cdot PesoDist_{bk} + DistMed_{cl}\cdot PesoDist_{cl}}{NumDist_{bk} + NumDist_{cl}}
\label{DefDistMed}
\end{equation} 
...
do genético (\textit{fitness}), ilustrando o uso de um fator de calibragem ($k$) a fim de tornar $M$ e $T$ compatíveis em termos de ordem de grandeza.
