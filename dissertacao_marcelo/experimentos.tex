...
Além disso, estimamos que a esperança da distância média das populações ($E_d$) é satisfatoriamente aproximada por $E_t = A_d/3$. Onde $A_d = 100$ pode ser
...
Assim, na rede de $30$ em estudo, o fator de calibragem adotado foi $C_a = 1740$. Mas vale resaltar que estas estimativas devem ser revistas para redes
...
milhoes de soluções. A hierarquia obtida, que representa a melhor solução visitada pelo genético, está representada na
tabela
...
A partir de testes piloto, estimou-se que as estatísticas atingiam estatabilidade a partir de $10^3$ amostras, para a confiança e o erro adotados. Portanto,
...
Os \textit{upper bounds} foram definidos como a média menos uma quantiade de desvios padrão, tanto para o congestionamento ($HmDs$) quanto para o tráfego
...
produzir soluções com multiplas ligações físicas, com o intuito de reduzir o número de variáveis binárias no modelo. Entretanto ainda são adimitidas
soluções com multiplas ligações lógicas, pois no TWA isso não interfere no número de variáveis. 
...
possível encontrar solução viável. Adicionalmente, aumentamos o \textit{TIFgap} enquando ainda fossem encontradas soluções, visando obter o melhor custo
