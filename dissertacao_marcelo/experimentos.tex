...
testes computacionais. Como o foco principal deste trabalho consistiu no desenvolvimento da metodologia de projeto, não tendo visado gerar expressivos
resultados a partir de parametrizações e testes variados, será analisada somente uma rede e apresentadas as conclusões sobre este caso específico de estudo.

Serão apresentados os detalhes dos testes realizados com uma rede de $30$ nós. As matrizes de tráfego e distâncias, dados de entrada do
...
Sabe-se que $A_t = 100$, equivale à amplitude do intervalo de onde foram retiradas as entradas da matriz de demandas, neste caso $100$.
...
Assim, na rede de $30$ nós em estudo, o fator de calibragem adotado foi $C_a = 1740$. Mas vale ressaltar que estas estimativas devem ser revistas para redes
...
Para os operadores genéticos da \textit{GAlib}, as probabilidades de mutação, \textit{crossover} e substituição foram mantidas em seus valores
padrão. O
...
configurações de hierarquia para a rede. Pode-se notar que o melhor indivíduo retornado pelo algoritmo, com $M = 24.4$, possui uma boa configuração
com
...
\begin{figure}[!htb]
\begin{center}
 %\includegraphics[angle=0,scale=.7]{./figs/c4.DistMed.eps}
 \includegraphics[angle=0,scale=.7, bb=0 0 539 540]{./figs/c4.DistMed.eps}
...
\begin{figure}[!htb]
\begin{center}
 %\includegraphics[angle=0,scale=.7]{./figs/30.I1.s1.individuos-c4.TrafR_.eps}
 \includegraphics[angle=0,scale=.7, bb=0 0 539 540]{./figs/30.I1.s1.individuos-c4.TrafR_.eps}
 % hist.dist.eps: 595x842 pixel, 72dpi, 20.99x29.70 cm, bb=0 0 595 842
...
Um ponto que pode ser destacado é que para o congestionamento, o LI calculado possui boa qualidade visto que é bastante próximo dos
...
\textit{backbone}, não se pode afirmar que o LI calculado possui muita qualidade.
...
Conforme descrito na seção anterior, a limitação de custo físico foi definida com base na TIF da rede real NFSNET, como definido na equação \ref{DefUBCFis}.
Os
...
forçar uma boa qualidade no resultado do projeto da rede física. Na coluna TIF da tabela \ref{tab:result.twa}, está apresentado o valor da TIF calculado
para a
...
Nos experimentos o custo físico não apresentou relação evidente com o grau lógico. Isso fica claro pois para cada sub-rede o valor de $TIFgap$
para o
