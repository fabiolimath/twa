...
Seção \ref{sec:EstruturaHierarquica}. Definimos $M_i = 4$, o tamanho mínimo dos \textit{clusters}, e $Q_a = M_a = \lfloor n/M_i\rfloor =
30/3 = 10$, respectivamente, o número máximo de \textit{clusters} e seu tamanho máximo.
...
1. Pode-se notar que neste caso, para ambas as estatísticas, $10^3$ amostras foram suficientes para a convergência. Mais adiante será mostrado que tanto para o
congestionamento quanto para o tráfego retransmitido, no caso de grau lógico 1, esse tamanho de amostra foi suficiente para todas as sub-redes, segundo o
...
CongDs & $=$ & Desvios padrão para o UB de congestionamento \\
UBCong & $=$ & \textit{Upper bound} para congestionamento \\
TRetrDs & $=$ & Desvios padrão para o UB de tráfego retransmitido \\
...
retransmitido, respectivamente. As cinco colunas que a seguem, são apresentadas em valores percentuais relativamente ao \textit{lower bound}, mais
especificamente, quantos por cento acima do mesmo, com exceção do desvio padrão, coluna $DP$, que é uma relação percentual direta. Como \textit{lower bound}
...
Uma evidência da qualidade das amostras de topologia visitadas para estimação das estatísticas, é a comparação dos mínimos amostrais com a heurística HLDA, em
todos os casos a melhor topologia sorteada obteve resultado melhor ou igual ao HLDA. Outro ponto de destaque é que para o congestionamento, o \textit{lower
bound} calculado possui excelente qualidade visto que é muito próximo dos mínimos amostrais. Quanto ao tráfego retransmitido, o mesmo não se observou, a não
ser para o \textit{backbone} onde os mínimos amostrais se aproximaram do \textit{lower bound}.
...
Nas duas tabelas observa-se casos onde a grande variabilidade das topologias visitadas leva a desvios padrão superiores a $50\%$. Para o congestionamento,
chamam a atenção as instâncias que possuem desvio padrão superior a média.

Para o caso de grau lógico 1, rede em anel, destaca-se a pequena diferença entre máximo e mínimo. A pouca variabilidade desse tipo de topologia é reforçada pelo
pequeno desvio padrão observado nestas instâncias.
...
Conforme definido nas equações \ref{DefUBCong} e \ref{DefUBTRetr}, os \textit{upper bounds} foram calculados como a média menos uma quantidade de desvios
...
múltipla de $1/2$ e foram definidas como a maior possível, desde que o \textit{upper bound} resultante não ultrapasse o respectivo mínimo amostral. A tabela
\ref{tab:ubs} mostra os valores resultantes desta regra.

\begin{equation}
UBCong = MED - CongDs\cdot DP
\label{DefUBCong}
\end{equation} 

\begin{equation}
UBTRetr = MED - TRetrDs\cdot DP
\label{DefUBTRetr}
\end{equation} 
...
\hline ID & GL & CongDs & UBCong & TRetrDs & UBTRetr \\
...
Para os casos em que $CongDs = 0$, significa que o \textit{upper bound} para o congestionamento foi definido como a própria média amostral ($MED$).


Além dos parâmetros de controle $UBCong$ e $UBTRetr$, que obtemos a partir das estatísticas, para garantir que a solução obtida para a topologia física com
o
...
dessa forma definir o \textit{upper bound} para o custo físico ($UBCFis$) de cada sub-rede. Conforme será detalhado na próxima seção, o $TIFgap$ utilizado
variou entre $70\%$ e $20\%$. 

\begin{equation}
UBCFis = TIF_{NFSNET}\cdot (1-TIFgap)\cdot \sum_{mn} C_{mn}
\label{DefUBCFis}
\end{equation} 

A equação \ref{DefUBCFis} define a fórmula de cálculo para $UBCFis$. Como se pode notar, partindo de um valor de referência ($TIF_{NFSNET}$), um fator de
...
\textit{upper bound} para cada instância do problema.
...
Os valores de \textit{upper bounds} utilizados para tráfego retransmitido e congestionamento são os apresentados na tabela \ref{tab:ubs}. Com relação à
...
possível que ainda atendesse às demais exigências.
...
rapidamente, sacrificando um pouco a qualidade das restrições. Essas situações em que o modelo precisou ser calibrado, que chamaremos de \textit{Instância
...
anteriormente em \cite{SBPO2009}.
...
$\hspace{10pt}$ $\text{Matrizes de tráfego e distância}$; $UBCong$; $UBTRetr$; $TIF_{NFSNET}$ \\
...
$\hspace{10pt}$ \underline{9} $\hspace{90pt}$ \textbf{GO TO} \underline{step (3)} \\
$\hspace{10pt}$ \underline{10} $\hspace{53pt}$ \textbf{END IF} \\
$\hspace{10pt}$ \underline{11} $\hspace{53pt}$ \textbf{IF} $\text{Solução Viável}$ \textbf{AND} $TIFgap \geqslant 40\%$ \\
$\hspace{10pt}$ \underline{12} $\hspace{83pt}$ $TIFgap = TIFgap + 10\%$; \\
$\hspace{10pt}$ \underline{13} $\hspace{83pt}$ \textbf{GO TO} \underline{step (3)} \\
$\hspace{10pt}$ \underline{14} $\hspace{53pt}$ \textbf{END IF} \\
$\hspace{10pt}$ \underline{15} $\hspace{23pt}$ \textbf{END FOR} 
\label{pseudocod:TWA}
\end{pseudocod}
...
TIFgag & $=$ & Folga em relação à TIF da NFSNET \\
...
O número de comprimentos de onda ($W$) necessários para atender às demandas de tráfego, que é um fator relevante do ponto de vista de complexidade e custo de
uma rede, variou entre $1$ e $4$.
...
valores de $UBCFis$, foram calculados a partir desse valor de referência para a TIF, menos um percentual $TIFgap$, que representa uma tentativa de
...
solução obtida para a topologia física.
...
Para duas instâncias, $c1.t5$ e $c3.t7$, ambas com grau lógico 1, mesmo com um $TIFgap$ reduzido a $0.0\%$ não foi possível encontrar solução viável. Então
relaxamos os \textit{upper bounds} para congestionamento e tráfego retransmitido, reduzindo em $1/2$ o número de desvios padrão abaixo da média para ambos, nas
duas instâncias. Os valores mostrados na tabela \ref{tab:ubs} foram apresentados com essas exceções.
...
qual foi possível obter soluções viáveis foi o mesmo, independente do grau lógico.

No modelo TWA o grau lógico é representado como uma restrição no número total de transceptores na rede. Como se trata de uma limitação, existem
casos onde não são utilizados todos os transceptores permitidos pelo grau lógico definido. Na tabela \ref{tab:result.twa}, como pode-se notar na coluna
$TTranc$, além do grau lógico $1$ onde é
óbvio que o número de transceptores deve ser igual ao número de nós, somente para o grau lógico $2$, em algumas sub-redes, foi necessário utilizar todos os
transceptores permitidos. As três colunas que seguem, $CFis$, $TRetr$ e $Cong$, trazem os resultados obtidos para custo físico, tráfego retransmitido e
congestionamento, respectivamente.
...
$TIFgap$. Além disso, para os casos de exceção citados acima, onde foi necessário folgar os \textit{upper bounds} de tráfego e congestionamento, também foi
contabilizado o tempo das tentativas com os valores originais de desvios padrão abaixo da média ($CongDs$ e $TRetrDs$). A coluna $Ti(s)$ mostra o tempo
relativo apenas à última tentativa de resolução, ou seja, àquela que retornou uma solução viável. Como pode-se notar, na maioria dos casos esse último tempo
...
A figura \ref{fig:twa.ftnet} mostra o comportamento do tráfego retransmitido em cada sub-rede de acordo com o grau lógico. Além disso, para evidenciar a
