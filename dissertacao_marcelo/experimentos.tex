...
configuração da rede. Para a distância a diferença entre a pior e a melhor solução visitada foi superior a $100\%$, enquanto para o tráfego de acesso essa
diferença foi menor que $30\%$. \underline{\textbf{Conferir esses percentuais.}}
...
Além dos parâmetros de controle que obtemos a partir das estatísticas ($UBCong$ e $UBTRetr$), para garantir que a solução obtida para a topologia física com o
...
$\hspace{10pt}$ \textbf{Entrada:} Dados da sub-rede ($\text{Matrizes de tráfego e distância}$; $UBCong$; $UBTRetr$; $TIF_{NFSNET}$) \\
$\hspace{10pt}$ \underline{1} $\hspace{30pt}$ \textbf{FOR} $Gl=1$ \textbf{TO} $Gl=\left[t_k/2\right]$ \textbf{DO} \\
...
Além de calcular congestionamento e tráfego retransmitido para as estatísticas, como foi descrito na seção \ref{sec:subredes:estatisticas}, o GLPK
...
forçar uma boa qualidade no resultado do projeto da rede física. Na coluna TIF, está apresentado o valor da TIF calculado para a solução obtida para a topologia
física.
...
casos onde não são utilizados todos os transceptores permitidos pelo grau lógico definido. Como pode-se notar na coluna $TTranc$, além do grau lógico $1$ onde é
...
global da rede em hierarquia, ela destaca o tráfego de acesso ao \textit{backbone} que foi calculado na etapa inicial, quando o genético fez a escolha das
sub-redes.
