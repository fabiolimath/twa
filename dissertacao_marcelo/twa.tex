...
descrição está reproduzida parcialmente neste capítulo. Será apresentada uma adaptação do modelo que foi
utilizada nos experimentos computacionais do capítulo \ref{result}. O TWA é capaz de tratar desde a escolha
da topologia física de uma WRON até a definição da
...
Na Restrição \ref{rest:DefCapFlow}, a conservação dos percursos lógicos é feita da seguinte forma: a soma dos componentes das ligações lógicas iniciadas em
um nó $i$ no comprimento de
...
\ref{eq:custoInstlacao}). Esta restrição foi modificada, em relação ao modelo original, com o objetivo de exigir que todo enlace físico seja bidirecional,
ou seja, é assumido que será instalado um par de fibra, sendo elas utilizadas em sentidos opostos de propagação.
