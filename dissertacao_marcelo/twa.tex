...
O modelo TWA é introduzido em \cite{dissertFabio}, onde também são apresentados diversos casos de uso. Esta seção descreve uma adaptação do modelo,
assumindo ligações físicas bidirecionáis, utilizada nos experimentos computacionais do Capítulo \ref{resultados}. O TWA é capaz de tratar desde a escolha da
topologia física de uma WRON até a definição da topologia virtual, incluindo a distribuição de tráfego, a definição das rotas físicas e a alocação de
comprimentos de onda. Ainda assim, possui um reduzido número de variáveis e restrições, se comparado a modelos que resolvem apenas o RWA, como os que são
tratados em \cite{Jaumard04}. Na literatura o projeto completo, incluindo topologias física e lógica, foi modelado em \cite{Xin:03}, possuindo uma
complexidade elevada, que torna o uso de
...
As Variáveis \ref{FisVar} e \ref{FlowVar} completam as definições necessárias para apresentarmos a forma básica do modelo TWA.
...
facilitar a compreensão das restrições do modelo, que ficam menos dependentes do dados de entrada. A outra diferença é que o fluxo é separado por
